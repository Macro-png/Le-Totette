...
def crearCliente(di):
    """
    Inserta un nuevo cliente. di debe contener: nombre, mail, contrasena
    El campo tipo se inserta por defecto como 'cliente'.
    """
    sql = "INSERT INTO clientes (nombre, tipo, mail, contrasena) VALUES (%s, %s, %s, %s)"
    val = (di.get('nombre'), 'cliente', di.get('mail'), di.get('contrasena'))
...
def crearProducto(di):
    ...
    sql = """INSERT INTO productos (nombre, precio_unidad, img, descripcion, ventas)
             VALUES (%s, %s, %s, %s, %s)"""
    val = (di.get('nombre'), di.get('precio_unidad') or 0, di.get('img') or '', di.get('descripcion') or '', 0)
    return insertDB(BASE, sql, val) == 1

def obtenerProductos():
    sql = "SELECT id, nombre, precio_unidad, img, descripcion, ventas FROM productos ORDER BY id DESC"
...
def guardarTotePersonalizado(clientes_id, img_name, color=None, estampa=None):
    """
    Guarda una personalización (sin asociar aún a un pedido).
    Tabla: detalle_personalizados (clientes_id, img, color, estampa)
    """
