...
def selectDB(configDB=None,sql="",val=None,title=False):
    ...
    resQuery=None
    if configDB!=None:
        mydb=conectarBD(configDB) #ABRO CONEXION
        resQuery=consultarDB(mydb,sQuery=sql,val=val,title=title)
        cerrarBD(mydb) #CIERRO CONEXION
    return resQuery
...
def insertDB(configDB=None,sql="",val=None):
    ...
    res=None
    if configDB!=None:
        mydb=conectarBD(configDB) #ACA SE ABRE CONEXION
        res=ejecutarDB(mydb,sQuery=sql,val=val)
        cerrarBD(mydb) #ACA SE CIERRA CONEXION
    return res
